...
@st.cache_data
def load_data1():
    day_df = pd.read_csv("dashboard/dayz.csv")
    day_df["dteday"] = pd.to_datetime(day_df["dteday"], errors='coerce')  # Pastikan kolom dteday dalam format datetime
    return day_df

@st.cache_data
def load_data2():
    hour_df = pd.read_csv("dashboard/hourz.csv")
    hour_df["dteday"] = pd.to_datetime(hour_df["dteday"], errors='coerce')  # Pastikan kolom dteday dalam format datetime
    return hour_df
